---
title: "NO2 Lombardia, cluster analysis serie gennaio-marzo"
description: |
  Serie gennaio-marzo 2020.
author:
...
Cluster analysis (usando Dynamic Time Warping, DTW) delle serie di `r params$inquinante` per il periodo Gennaio-Marzo 2020, regione `r params$regione`. 
...
leaflet() %>%
  addTiles() %>%
  addAwesomeMarkers(data=prova %>% filter(cluster==1),lng=~st_x,lat=~st_y,group = "Cluster1",icon = icon1,popup = ~zona_tipo) %>%
  addAwesomeMarkers(data=prova %>% filter(cluster==2),lng=~st_x,lat=~st_y,group = "Cluster2",icon = icon2,popup = ~zona_tipo) %>%
  addAwesomeMarkers(data=prova %>% filter(cluster==3),lng=~st_x,lat=~st_y,group = "Cluster3",icon = icon3,popup = ~zona_tipo) %>%  
  addAwesomeMarkers(data=prova %>% filter(cluster==4),lng=~st_x,lat=~st_y,group = "Cluster4",icon = icon4,popup = ~zona_tipo) %>%  
  addLayersControl(overlayGroups=c("Cluster1","Cluster2","Cluster3","Cluster4"),layersControlOptions(options=list(collapse=FALSE)))
```  
...
#### Cluster3: tipo_stazione

```{r conta_cluster333}
table(prova[prova$cluster==3,c("zona_tipo")])
```

#### Cluster4: tipo_stazione

```{r conta_cluster444}
table(prova[prova$cluster==4,c("zona_tipo")])
```
...
st_distance(st_prova %>% filter(cluster==3))->matriceDistanze
as.numeric(matriceDistanze[lower.tri(matriceDistanze)])->dist_clust3

st_distance(st_prova %>% filter(cluster==4))->matriceDistanze
as.numeric(matriceDistanze[lower.tri(matriceDistanze)])->dist_clust4
```
...
```{r distanze3}
skimr::skim(dist_clust3)
```

#### Cluster4

```{r distanze4}
skimr::skim(dist_clust4)
```
