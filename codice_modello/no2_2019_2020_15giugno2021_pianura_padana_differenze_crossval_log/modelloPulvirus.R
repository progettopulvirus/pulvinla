...
stazioni %>%
  mutate(tipo_zona=ifelse(tipo_zona=="R-remote","R",tipo_zona))->stazioni
...
purrr::walk(LISTA_MESI,.f=function(MESE){
  ...
  stazioni %>%
    group_split(tipo_zona)->listaStazioni
  
  purrr::imap_dfr(listaStazioni,.f=function(df,.y){
  ...
  partition(campione$tipo_zona,type="stratified",p=c(0.33,0.33,0.33))->indici
  campione[indici$`1`,]->val1
  campione[indici$`2`,]->val2
  campione[indici$`3`,]->val3
  
  purrr::iwalk(list(val1,val2,val3),.f=function(validazione,indice){
  ...
  validazione$station_eu_code->codVal
  setdiff(unique(dati$station_eu_code),codVal)->codTrain

  #training
  dati[dati$station_eu_code %in% codTrain,]->datiTrain
  inla.spde.make.A(mesh=mesh,loc=as.matrix(coordinateOsservazioni[coordinateOsservazioni$station_eu_code %in% codTrain,c("coordx_km","coordy_km")]),group =datiTrain$banda,n.spde=spde$n.spde,n.group =n_giorni )->A.training
  
  #validation
  dati[dati$station_eu_code %in% codVal,]->datiVal
  inla.spde.make.A(mesh=mesh,loc=as.matrix(coordinateOsservazioni[coordinateOsservazioni$station_eu_code %in% codVal,c("coordx_km","coordy_km")]),group =datiVal$banda,n.spde=spde$n.spde,n.group =n_giorni )->A.validation
  ...
  inla.stack(data=list(value=datiTrain$value),A=list(A.training,1),effects=list(iset,datiTrain[c(EFFETTI)]),tag="training")->stack.training
  saveRDS(stack.training,glue::glue("stack.training{MESE}_indice{indice}_{REGIONE}.RDS"))
  
  inla.stack(data=list(value=NA),A=list(A.validation,1),effects=list(iset,datiVal[c(EFFETTI)]),tag="validation")->stack.validation
  saveRDS(stack.validation,glue::glue("stack.validation{MESE}_indice{indice}_{REGIONE}.RDS"))
  
  inla.stack(stack.training,stack.validation)->mystack
  saveRDS(mystack,glue::glue("mystack{MESE}_indice{indice}_{REGIONE}.RDS"))
  ...
  inla.out$summary.fitted.values$mean[righe]->datiTrain$fitted
  inla.out$summary.fitted.values$`0.025quant`[righe]->datiTrain$fitted025
  inla.out$summary.fitted.values$`0.975quant`[righe]->datiTrain$fitted975
  
  datiTrain %>% 
    mutate(residui=value-fitted) %>%
    saveRDS(.,glue::glue("datiTrain{MESE}_indice{indice}_{REGIONE}.RDS")) 
  
  rm(righe)
  
  #calcolo residui su scala logaritmica
  mystack$data$index$validation->righe
  inla.out$summary.fitted.values$mean[righe]->datiVal$fitted
  inla.out$summary.fitted.values$`0.025quant`[righe]->datiVal$fitted025
  inla.out$summary.fitted.values$`0.975quant`[righe]->datiVal$fitted975
  
  # 
  datiVal %>% 
    mutate(residui=value-fitted) %>%
    saveRDS(.,glue::glue("datiVal{MESE}_indice{indice}_{REGIONE}.RDS")) 
  
  }) #fine walk su validazione
