...
inla.posterior.sample.eval(function(...,A=Aloc,df=dati){
  ...
  meteo<-t2m*df$t2m+tp*df$tp+ptp*df$ptp+sp*df$sp+wspeed*df$wspeed+pbl12*df$pbl12+pbl00*df$pbl00+pwspeed*df$pwspeed+rh*df$rh
...
write_delim(dati2,glue::glue("samples{MESE}_meteo.csv"),delim=";",col_names=TRUE)
