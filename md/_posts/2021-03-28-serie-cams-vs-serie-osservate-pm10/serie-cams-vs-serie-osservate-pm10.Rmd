---
title: "Serie CAMS vs serie osservate, PM10"
description: |
  PM10, serie Lombardia gennaio-febbraio 2020.
author:
...
cams %>%
  dplyr::select(station_eu_code,yy,mm,dd,date,counter_pm10) %>%
...
dati %>%
  ...
  summarise(correlazione=cor(value,counter_pm10,use = "pairwise.complete.obs"))->correlazioni
...
dati %>%
  group_by(station_eu_code) %>%
  summarise(correlazione=RMSE(value,counter_pm10))->rmse
```
...
ggplot(data=dati,aes(x=value,y=counter_pm10))+
  geom_point(alpha=0.25)+
  geom_abline(a=0,b=1,col="red")+
  geom_smooth(aes(x=value,y=counter_pm10),se=FALSE)
```
...
ggplot(data=dati)+
  ...
  geom_line(aes(x=date,y=counter_pm10),colour="red")+
