...
###############################
METEO<-c("t2m","tp","ptp","sp","wspeed","pblmax","pblmin","dtr")
...
left_join(dati,cams %>% dplyr::select(-matches("pm"))) %>%
  rename(cams=counter_no2)->dati
update(myformula,.~.+cams)->myformula
...
purrr::imap(CODICI,.f=function(.codice,i){
  ...
  X_est = X %>% mutate(bandalockdown =lockdown*banda)
