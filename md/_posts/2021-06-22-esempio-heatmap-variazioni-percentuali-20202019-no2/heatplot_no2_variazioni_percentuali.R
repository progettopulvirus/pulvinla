...
dati %>%
  ...
  dplyr::select(`2019`,`2020`,station_eu_code,banda,wday,mm,regione,coordx,coordy,evalue)  %>%
  mutate(indice=banda+offset)
...
finale %>%
  mutate(evalue=ifelse(abs(evalue)>1,1,evalue))->finale


ggplot(data=finale,aes(x=indice,y=station_eu_code,fill=evalue))+
  geom_tile( )+
