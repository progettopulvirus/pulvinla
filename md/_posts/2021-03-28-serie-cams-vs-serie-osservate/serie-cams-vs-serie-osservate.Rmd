---
title: "Serie CAMS vs serie osservate"
description: |
  NO2, serie Lombardia gennaio-febbraio 2020.
author:
...
cams %>%
  dplyr::select(station_eu_code,yy,mm,dd,date,counter_no2) %>%
...
dati %>%
  ...
  summarise(correlazione=cor(value,counter_no2,use = "pairwise.complete.obs"))->correlazioni
...
dati %>%
  group_by(station_eu_code) %>%
  summarise(correlazione=RMSE(value,counter_no2))->rmse
```
...
ggplot(data=dati,aes(x=value,y=counter_no2))+
  geom_point(alpha=0.25)+
  geom_abline(a=0,b=1,col="red")+
  geom_smooth(aes(x=value,y=counter_no2),se=FALSE)
```
...
ggplot(data=dati)+
  ...
  geom_line(aes(x=date,y=counter_no2),colour="red")+
