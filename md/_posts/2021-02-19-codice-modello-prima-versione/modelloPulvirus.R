...
#16 luglio
list(theta = list(prior="pc.prec", param=c(0.5,0.01)))->prec_hyper
...
inla.stack(data=list(value=dati$value),A=list(A.training,1),effects=list(iset,dati[c("station_eu_code","banda2",attr(termini,"term.labels"))]),tag="training")->stack.training
saveRDS(stack.training,glue::glue("stack.training_{REGIONE}.RDS"))

########################
#Random effects
########################
update(myformula,.~.+f(station_eu_code,model="iid")+f(banda2,model="rw1",hyper=prec_hyper)+f(i,model=spde,group = i.group,control.group = list(model="ar1",hyper=list(theta=theta_hyper))))->myformula
